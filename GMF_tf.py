...
def get_model(num_users, num_items, latent_dim, regs=[0,0]):
    ...
    MF_Embedding_User = tf.keras.layers.Embedding(input_dim = num_users, output_dim = latent_dim, name = 'user_embedding',
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(regs[0]), input_length=1)
    MF_Embedding_Item = tf.keras.layers.Embedding(input_dim = num_items, output_dim = latent_dim, name = 'item_embedding',
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(regs[1]), input_length=1)   
    ...
    predict_vector = tf.keras.layers.Multiply()(inputs=[user_latent, item_latent])
    
    # Final prediction layer
    #prediction = Lambda(lambda x: K.sigmoid(K.sum(x)), output_shape=(1,))(predict_vector)
    prediction = tf.keras.layers.Dense(1, activation='sigmoid', kernel_initializer='lecun_uniform')(predict_vector)
