...
import sys
import GMF_tf as GMF, MLP_tf as MLP
import argparse
...
def get_model(num_users, num_items, mf_dim=10, layers=[10], reg_layers=[0], reg_mf=0):
    ...
    MF_Embedding_User = tf.keras.layers.Embedding(input_dim = num_users, output_dim = mf_dim, name = 'mf_embedding_user',
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(reg_mf), input_length=1)
    MF_Embedding_Item = tf.keras.layers.Embedding(input_dim = num_items, output_dim = mf_dim, name = 'mf_embedding_item',
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(reg_mf), input_length=1)   

    MLP_Embedding_User = tf.keras.layers.Embedding(input_dim = num_users, output_dim = layers[0]/2, name = "mlp_embedding_user",
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(reg_layers[0]), input_length=1)
    MLP_Embedding_Item = tf.keras.layers.Embedding(input_dim = num_items, output_dim = layers[0]/2, name = 'mlp_embedding_item',
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(reg_layers[0]), input_length=1)   
